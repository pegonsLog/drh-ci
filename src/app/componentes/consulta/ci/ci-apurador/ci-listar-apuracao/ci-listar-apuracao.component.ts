...
export class CiListarApuracaoComponent implements OnInit {
  ...
  loadCis(direction: 'next' | 'prev'): void {
    ...
    const cursor = direction === 'next' ? this.lastDoc : this.firstDoc;
        ...
        this.ciService.getCisParaApuracao(this.pageSize, direction, cursor ?? undefined).subscribe((result: { cis: ComunicacaoInterna[], firstDoc: DocumentSnapshot<DocumentData> | null, lastDoc: DocumentSnapshot<DocumentData> | null }) => {
